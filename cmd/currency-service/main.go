...
func main() {
	flag.Usage = func() {
		fmt.Println("Верный формат ввода для конвертации: `calculate --amount 100 --rate 80`") // Допиши весь кли
	}
	if len(os.Args) < 2 {
		flag.Usage()
	...
	slogLevel, err := logging.ParseSlogLevel(level)
	if err != nil {
		errorSlogLogger := logging.New(slogLevel)
		errorSlogLogger.Error("undefined log level", "level", level, "err", err)
		os.Exit(1)
	}
	slogLogger := logging.New(slogLevel)
