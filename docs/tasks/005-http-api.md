...
Термины задачи: **HTTP-обработчик (handler)** — функция, которая принимает HTTP-запрос, вызывает прикладную логику и формирует ответ; это граница операции `serve`, она пишет ошибки в лог один раз. **`application`** — пакет `internal/application/` из задачи 004 с логикой конвертации; HTTP-слой только разбирает query-параметры и сериализует ответ, расчёт не дублирует.
