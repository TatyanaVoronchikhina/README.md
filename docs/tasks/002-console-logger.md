...
Термины задачи: **`operation`** — обязательное поле лога с именем выполняемой операции (`calculate`, позже `import`, `convert`, `serve`, `aggregate-day`); позволяет отличить записи разных команд в общей консоли. **Граница операции** — самый внешний слой команды (в этой задаче — обработчик `calculate` в `main`), который получает ошибку из нижних слоёв и пишет её в лог ровно один раз.
